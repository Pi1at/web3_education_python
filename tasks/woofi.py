...
class WooFi(Base):
    ...
    async def swap_usdc_to_eth(
        self, amount: Optional[TokenAmount] = None, slippage: float = 1
    ) -> str:
        ...
        await self.approve_interface(
            token_address=from_token.address, spender=contract.address, amount=amount
        )
        ...
        tx_params = TxParams(
            to=contract.address, data=contract.encodeABI("swap", args=args.tuple())
        )
