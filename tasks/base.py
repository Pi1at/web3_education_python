...
class Base:
    ...
    async def get_token_price(
        token_symbol="ETH", second_token: str = "USDT"
    ) -> float | None:
            ...
            try:
                async with aiohttp.ClientSession() as session:
                    async with session.get(
                        f"https://api.binance.com/api/v3/depth?limit=1&symbol={token_symbol}{second_token}"
                    ) as r:
                        if r.status != 200:
                            return None
        ...
        raise ValueError(
            f"Can not get {token_symbol + second_token} price from Binance"
        )

    async def approve_interface(
        self, token_address, spender, amount: TokenAmount | None = None
    ) -> bool:
        ...
        approved = await self.client.transactions.approved_amount(
            token=token_address, spender=spender, owner=self.client.account.address
        )
        ...
        tx = await self.client.transactions.approve(
            token=token_address, spender=spender, amount=amount
        )
