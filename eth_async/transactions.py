...
class Tx(AutoRepr):
    ...
    def __init__(
        self, tx_hash: str | _Hash32 | None = None, params: dict | None = None
    ) -> None:
        """
    ...
    async def wait_for_receipt(
        ...
        """
        self.receipt = dict(
            await client.w3.eth.wait_for_transaction_receipt(
...
class Transactions:
    ...
    async def sign_and_send(self, tx_params: TxParams) -> Tx:
        ...
        tx_hash = await self.client.w3.eth.send_raw_transaction(
    ...
    async def wait_for_receipt(
        ...
        """
        return dict(
            await self.client.w3.eth.wait_for_transaction_receipt(
    ...
    async def approve(
        ...
        gas_price: types.GasPrice | None = None,
        ...
        if not amount:
            ...
            amount = TokenAmount(
                amount=amount, decimals=await contract.functions.decimals().call()
            ).Wei
        ...
        tx_params = {
            ...
            "data": contract.encodeABI(
                "approve", args=TxArgs(spender=spender, amount=amount).tuple()
            ),
        }
        if self.client.network.tx_type == 2:
            tx_params["maxPriorityFeePerGas"] = (
                await self.client.transactions.max_priority_fee()
            ).Wei
            tx_params["maxFeePerGas"] = (
                gas_price.Wei + tx_params["maxPriorityFeePerGas"]
            )
