...
async def get_token_price(from_token, to_token) -> float | None:
    ...
    async with aiohttp.ClientSession() as session:
        async with session.get(
            f"https://api.binance.com/api/v3/ticker/price?symbol={from_token + to_token}"
        ) as r:
            result_dict = await r.json()
            if "msg" in result_dict and "Invalid symbol" in result_dict["msg"]:
                async with session.get(
                    f"https://api.binance.com/api/v3/ticker/price?symbol={to_token + from_token}"
                ) as r:
                    result_dict = await r.json()
...
async def main():
    ...
    woofi_contract = await client.contracts.get(
        contract_address=Contracts.ARBITRUM_WOOFI
    )

    from_amount = TokenAmount(amount=0.001)
    min_to_amount = TokenAmount(
        amount=float(from_amount.Ether)
        * await get_min_to_amount(from_token=from_token.title, to_token=to_token.title),
        decimals=6,
    )
